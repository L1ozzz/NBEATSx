...
space = [
    ...
    Integer(1, 6, name='n_blocks'),
    Integer(1, 10, name='n_layers'),
    Integer(127, 1500, name='n_hidden_units')  # 假设你想在500到1500之间搜索隐藏单元数量
...
def objective(**params):
    ...
    original_data = params.copy()
    print(original_data)

    n_blocks = params.pop('n_blocks')
    n_layers = params.pop('n_layers')
    n_hidden_units = params.pop('n_hidden_units')

    n_hidden = [[n_hidden_units] * n_layers for _ in range(5)]
    ...
    # 记录结果
    result = original_data  # 复制当前参数字典
    print(params)
    result['MAE'] = val_loss  # 添加MAE结果
    results_list.append(result)  # 添加到结果列表
